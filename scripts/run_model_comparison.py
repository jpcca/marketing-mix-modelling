...
def main():
    ...
    for spec in MODEL_SPECS:
        ...
        try:
            ...
            # Compute LOO
            loo_result = compute_loo(mcmc, spec.fn, x_train, y_train,
                                     prior_config, **spec.kwargs)
            
            # Compute predictions
            preds = compute_predictions(
                mcmc, spec.fn, x_train, y_train,
                x_test, prior_config, **spec.kwargs
            )
            
            # Compute metrics
            metrics_train = compute_predictive_metrics(preds["y_train_pred"], y_train)
            metrics_test = compute_predictive_metrics(preds["y_test_pred"], y_test)
    ...
    if len(df) > 1 and not df["elpd_loo"].isna().all():
